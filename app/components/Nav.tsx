...
export type Page = "home" | "activity" | "settings" | "send" | "receive" | "shield" | "unshield" | "swap" | "earn" | "more";
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-20 pb-6 pt-2 bg-gradient-to-t from-bg via-bg/95 to-transparent">
      <div className="max-w-md mx-auto px-8 flex items-center justify-around">
        {items.map(({ id, icon: Icon }) => {
          const active = current === id || (id === "home" && ["send","receive","shield","unshield","swap","earn","more"].includes(current));
          return (
            <button
              key={id}
              onClick={() => onChange(id)}
